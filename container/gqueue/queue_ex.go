...
func NewQueueEx(concurrentSafe bool, capacity int) *QueueEx {
	var result QueueEx
...
func (q *QueueEx) PushNoWait(x interface{}) (err error) {
	if q.concurrentSafe {
		q.inqMu.Lock()
	}
...
func (q *QueueEx) PushWait(x interface{}, timeout time.Duration) error {
...
func (q *QueueEx) PopNoWait() (x interface{}) {
	if q.concurrentSafe {
		q.inqMu.Lock()
	}
	if q.inq.Length() > 0 {
...
func (q *QueueEx) PopWait(timeout time.Duration) interface{} {
	ts := time.Now()
	for {
		if q.concurrentSafe {
			q.inqMu.Lock()
