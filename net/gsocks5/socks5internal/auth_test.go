...
func TestNoAuth(t *testing.T) {
	...
	out := resp.Bytes()
	if !bytes.Equal(out, []byte{Version, NoAuth}) {
		t.Fatalf("bad: %v", out)
	}
}
...
func TestPasswordAuth_Valid(t *testing.T) {
	...
	out := resp.Bytes()
	if !bytes.Equal(out, []byte{Version, UserPassAuth, 1, authSuccess}) {
		t.Fatalf("bad: %v", out)
	}
}
...
func TestPasswordAuth_Invalid(t *testing.T) {
	...
	out := resp.Bytes()
	if !bytes.Equal(out, []byte{Version, UserPassAuth, 1, authFailure}) {
		t.Fatalf("bad: %v", out)
	}
}
...
func TestNoSupportedAuth(t *testing.T) {
	...
	out := resp.Bytes()
	if !bytes.Equal(out, []byte{Version, noAcceptable}) {
		t.Fatalf("bad: %v", out)
	}
}
